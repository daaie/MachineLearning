...
# 텐서플로우가 텐서단위로 나눈이유는
# 오차보정을위해 가중치를 변경시켜줘야하는데
# 각 텐서들을 미분해서 오차를 보정할 수 있기 때문
# 중간 노드를 미분할수도있고 어떤노드든 골라서 미분할 수있다.
# 그래서 계산과정은 따라올라가는 식으로 되어있음.
# ph_1을 활용한 루트 -> 루트를 활용한 덧셈 이런식으로.
...
with tf.Session() as sess :
    ...
    # plus_tensor함수를 호출하는 것과 같다. 
    # 부를때만 값이있고 다른거 실행하면 사라짐.
    result = sess.run(plus_tensor, feed_dict={ph_1 : 17, 
                                              ph_2 : [10]})
    print(f"result = {result}")


    result = sess.run(squar_tensor_1, feed_dict={ph_1 : 3})
    print(f"result = {result}")
