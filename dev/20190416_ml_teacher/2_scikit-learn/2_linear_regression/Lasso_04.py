...
ls_model = Lasso(0.001).fit(X_train,y_train)
rf_model = RandomForestRegressor(n_estimators = 1000, max_depth=3, min_samples_leaf=3).fit(X_train,y_train)
...
print("LS학습 모델의 스코어 : ", ls_model.score(X_train, y_train))
print("RF학습 모델의 스코어 : ", rf_model.score(X_train, y_train))

print("LR테스트 모델의 스코어 : ", lr_model.score(X_test, y_test))
print("RG테스트 모델의 스코어 : ", rg_model.score(X_test, y_test))
print("LS테스트 모델의 스코어 : ", ls_model.score(X_test, y_test))
print("RF테스트 모델의 스코어 : ", rf_model.score(X_test, y_test))
...
plt.plot(coef_range, ls_model.coef_, 'go')
