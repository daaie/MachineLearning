...
# 그래디언트 부스팅은 데이터가 크면 클수록 학습이 잘 안됨  ->  그래서 XGboost 
...
#   과적합이 일어나지 않도록 제어
# - gamma: 정보 획득(Information Gain), 값이 클수록 트리 깊이가 줄어들어 
#   보수적인 모델이 된다. 디폴트 값은 0(반드시 조정해야함) -> max_depth와 비슷 
# - subsample: 각 트리마다의 관측 데이터 샘플링 비율, 
#   값을 작게 하는 경우 over-fitting을 방지하지만 너무 작은 값은 under-fitting.
#   일반적으로 0.5-1(디폴트 1)
# - max_depth: 각 트리의 maximum depth.  - 내부에서 디시전 트리를 쓰기때문
...
#   값이 커질 수록 보수적인 모델이 된다   - 더 범용적 
# - reg_alpha(L1 reg-form): L1 Regularization Form weights. 
#   값이 커질 수록 보수적인 모델이 된다 - 몇개 뺴고 다 0

# 3. 학습 과정 파라메터
# - objective: 목적 함수
#   <reg:linear, binary:logistic, multi:softmax, multi:softprob> // 다항분류 : softmax/softprob 똑같아..ㅋㅋ
...
#       reg_lambda=1 ==> L2제약조건 사용 , scale_pos_weight=1, seed=None, silent=True,
#       subsample=1) 
...
print("R2학습 결과 : ", model.score(X_train, y_train))

y_pred = model.predict(X_test)
mse = mean_squared_error(y_test, y_pred)
print("테스트 결과 : ", np.sqrt(mse))
print("R2테스트 결과 : ", model.score(X_test, y_test))

# r2 테스트 결과는 1에 가까울수록 좋은거임....
