...
# 2. 데이터의 전처리 및 머신러닝 모델 학습을 위한 파이프 라인 객체를 생성.
# Pipeline([(1번쨰 변환기 클래스 객체의 이름, 객체),(2번쨰 변환기 클래스 객체의 이름, 객체))])
# Pipeline([(1번쨰 변환기 클래스의 튜플),(2번쨰 변환기 클래스의 튜플))])
# 파이프 라인의 마지막 객체를 제외한 나머지 객체들은
# transform, fit_transform 메소드를 제공하는 변환기만 허용.
# 파이프 라인의 마지막 개체는 predict 메소드를 제공하는 예측기 객체가 될 수 있음.
    
    
# 파이프 라인의 실행 과정
# 1. fit 메소드가 호출되는 경우
# - 입력된 x 데이터를 첫번째 변환기 클래스의 객체로 전달(fit 메소드가 호출)
# - fit 메소드가 호출 된. 입력된 X 데이터를 transform 메소드를 통해서 변환 시킴.
# - 변환된 입력 데이터 x를 다음에 위치한 변환기 또는 예측기로 전달하여 fit메소드를 실행
# - 다음의 객체가 변환기 객체인 경우 trainsform 메소들의 실행 결과를 반환하여 다음 객체로 전달하고
# - 예측기 클래스인 경우 실행을 종료.
...
# 3. 파이프 라인의 실행을 통한 데이터 전처리 및 머신러닝 모델의 학습을 진행.
